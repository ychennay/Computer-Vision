...
c = OneVsRestClassifier(SVC()).fit(cluster_center_list,label_cluster_list).predict(cluster_center_list)

#calculate accuracy

#Linear SVM
float(sum(a == label_cluster_list)) / len(label_cluster_list)
#K Means
float(sum(b == label_cluster_list)) / len(label_cluster_list)
#Radial Basis Kernel SVM
